...
async function main() {
    ...
    data.push({language: motherLanguage, text: text, file: `${langPath}/${motherLanguage}/${motherLanguage}.mod_${ext['extension']}.ini`});
    ...
    const start = async () => {
        ...
        const langString = await getText(inputConst).then(
            async langStr => {
                ...
                langStr = fixLanguageConstant(langStr, ext['extension'], 'mod', ext['group'], constantType);
